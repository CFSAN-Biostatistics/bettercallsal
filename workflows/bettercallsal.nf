...
include { TREE_METADATA           } from "${params.modules}${params.fs}custom${params.fs}tree_metadata${params.fs}main"
            ...
            if (params.sourmashsketch_run) {
                ...
                if (params.skesa_run || params.megahit_run) {
                    ...
                    TOP_UNIQUE_SEROVARS
                        ...
                        .unique()
                        .collectFile( name: 'BCS_UNIQ_HITS.txt', newLine: true )
                        .ifEmpty( file('BCS_NO_UNIQ_HITS.txt') )
                        .map { hits_file ->
                            return [
                                [
                                    id: 'hitsTree',
                                    gds: ( "${params.tuspy_gds}" ?: false ),
                                    gd: ( "${params.tuspy_gd}" ?: false ),
                                    pdg_dir: ( "${params.tree_metadata_pdg_dir }" ?: false)
                                ],
                                hits_file
                            ]
                        }
                        .set { ch_bcs_uniq_hits }

                    TREE_METADATA( ch_bcs_uniq_hits )

                    MASHTREE(
                        TREE_METADATA.out.fofn,
                        ch_asm_filtered_contigs
                            .map { meta, contigs ->
                                contigs.toString()
                            }
                            .collectFile( name: 'ASM_FILTERED_CONTIGS.txt', newLine: true)
                    )
                    ...
                    software_versions
                        .mix (
                            MASHTREE.out.versions.ifEmpty(null),
            ...
            DUMP_SOFTWARE_VERSIONS (
                software_versions
                    .mix (
                        ...
                        TREE_METADATA.out.versions.ifEmpty(null),
